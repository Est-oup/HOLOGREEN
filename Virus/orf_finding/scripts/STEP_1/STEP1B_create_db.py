...
def create_blast_db(input_folder, output_folder, max_workers=None):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        for file_name in os.listdir(input_folder):
            ...
            output_file = os.path.join(output_subfolder, os.path.splitext(file_name)[0])
            print(f"Queueing: {input_file} -> {output_file}")
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python STEP1B_create_db.py <input_folder> <output_folder>")
        sys.exit(1)
