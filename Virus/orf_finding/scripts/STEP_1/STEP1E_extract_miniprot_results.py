...
def write_combined_fasta(sequences, output_file):
    with open(output_file, 'w') as file:
        for i, (seq, header) in enumerate(sequences):
            contig_name = header.split("_")[0]  # Extract contig name
...
def process_files(input_dir, output_dir, combined_file, log_file):
    ...
    with open(log_file, 'w') as log:
        ...
        for root, dirs, files in os.walk(input_dir):
            for name in files:
                if name.endswith(".gff"):
                    ...
                    if sequences:
                        protein_type = os.path.basename(root)
                        ...
                        headers = [f"{os.path.splitext(name)[0]}_{i+1}" for i in range(len(sequences))]
                        write_fasta(list(zip(sequences, headers)), output_file)
        ...
        for protein_type, seqs in protein_sequences.items():
            ...
            write_fasta(seqs, protein_output_file)
