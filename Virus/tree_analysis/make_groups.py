...
# Charger le fichier Newick (indépendamment de l'extension)
...
def process_newick_file(newick_file, output_file):
...
# Fonction principale pour traiter plusieurs fichiers dans un dossier
def main(input_folder, output_folder):
    # Créer le dossier de sortie s'il n'existe pas
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    
    # Parcourir tous les fichiers Newick dans le dossier d'entrée
    for filename in os.listdir(input_folder):
        if filename.endswith(".nwk") or filename.endswith(".newick") or filename.endswith(".contree"):
            newick_file = os.path.join(input_folder, filename)
            output_file = os.path.join(output_folder, f"{os.path.splitext(filename)[0]}_contig_groups.txt")
            print(f"Processing {newick_file}...")
            process_newick_file(newick_file, output_file)

# Utilisation
if __name__ == "__main__":
    input_folder = r'G:\Virus\NCLDV\viral_marker_annotation\\new_phylogeny_iqtree\\out\\STEP_7\\tree_data_filt\\'  # Dossier contenant les fichiers Newick
    output_folder = 'G:/Virus/NCLDV/arbres_rigoux_iqtree\\tree_group\\'  # Dossier pour les résultats
