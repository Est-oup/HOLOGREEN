...
#Estimation et remplacement des valeurs NA 
physeq@sam_data$Chl_a <- ifelse(is.na(physeq@sam_data$Chl_a),
                                       median(physeq@sam_data$Chl_a, na.rm = TRUE),
                                       physeq@sam_data$Chl_a)

#choisir les variables environnementales voulu
meta_RDA_init <- data.frame(sample_data(physeq))[, c(6,7,12:16,18,21:22)]

#normalizaton of the meta table (z-score)
meta_RDA_init[,-c(1,2)] <- vegan::decostand(meta_RDA_init[,-c(1,2)], method = "standardize")


#transformation des abondances 
...
tmp <- zCompositions::cmultRepl(physeq@otu_table,#ici les asv sont en colonne 
                                method = "CZM",
                                label = 0)

# generate the centered log-ratio transformed. ASVs are in rows!!!!! ici ca la transforme tout seul en asv row 
...
# RDA
rda_result <- vegan::rda(t(physeq_clr_asv) ~ .,meta_RDA_init[,-c(1,2)])
...
# Afficher les résultats de la sélection vers l'avant
print(step_forward)

#retrait chla
meta_RDA <-meta_RDA_init[, !(names(meta_RDA_init) %in% c("type_enrich", "Chl_a", "date", "ammonium"))]
...
res_st <- merge(res_st, meta_RDA_init[,c("type_enrich", "date")], by = "row.names")
...
RDA_plot <- ggplot() +
  ...
  labs(x = paste("RDA 1 (", round(as.numeric(eigen_values[1]),1), "%)", sep = ""),
       y = paste("RDA 2 (", round(as.numeric(eigen_values[2]),1), "%)", sep = "")) +
...
# 
# 
# meta_RDA[,-c(1,2)] <- apply(meta_RDA[,-c(1,2)], 1, function(x) log(x) - mean(log(x)))
